...
//returns the point where the lines extending from the segments intersects the line.
//returns undefined if the lines are parallel .
//can pass in 2 pairs of segment end points, or two Segment instances.
...
//returns the point where the first segment intersects the second segment.
//returns undefined if the segments do not intersect.
//can pass in 2 pairs of segment end points, or two Segment instances.
export function getSegmentIntersection(a1, a2, b1, b2) {
  ...
  let isOnY = Math.min(a1.y, a2.y) <= v.y && v.y <= Math.max(a1.y, a2.y);
