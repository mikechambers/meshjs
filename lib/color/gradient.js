...
import { randomInt, map } from "../math/math.js";
...
export default class Gradient {
  constructor(bounds, type = Gradient.LEFT_TO_RIGHT) {
    ...
    switch (type) {
      ...
      case Gradient.BOTTOM_RIGHT_TO_TOP_LEFT:
        x0 = bounds.width;
        y0 = bounds.height;
        x1 = bounds.x;
        y1 = bounds.y;
    ...
    this._gradient = this._context.createLinearGradient(x0, y0, x1, y1);
  }
  ...
  create() {
    ...
    this._pixelData = new PixelData(
      this._context.getImageData(x, y, w, h),
      false
    );
...
Gradient.TOP_LEFT_TO_BOTTOM_RIGHT = 5;
Gradient.BOTTOM_RIGHT_TO_TOP_LEFT = 6;
...
export function randomGradient(bounds, type) {
  ...
  return gradientFromName(name, bounds, type);
}
