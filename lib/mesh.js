...
import { createFileName, downloadJSON, downloadSVG } from "./data/datautils.js";
...
const DEFAULT_CONFIG = {
  ...
  //where to capture commands to enable SVG export. Note, this is an experimental
  //feature, not all canvas / context calls are support, and it can use a larger
  //amount of memory depending on content
  CAPTURE_SVG: false,
...
class MeshJS {
  constructor() {
    ...
    this._keyCommands = new Map([
      ...
      [`${mod}-${this._shortCutExportSVG}`, "Export SVG (if enabled)"],
      /*[
  ...
  init(config, initCallback, drawCallback, promise) {
    ...
    if (this._config.CAPTURE_SVG && !this._config.BATCH_DRAW_COMMANDS) {
      console.log(
        "Warning : config.CAPTURE_SVG requires that config.BATCH_DRAW_COMMANDS be set to TRUE."
      );
    }
  ...
  onKeyUp(event) {
    ...
    } else if (key === this._shortCutExportSVG) {
      this.downloadSVG();
    }
  }

  downloadSVG() {
    if (!this._config.CAPTURE_SVG) {
      console.log("Warning: CAPTURE_SVG must be set to true to export SVG.");
      return;
    }

    let svg = this._canvas.context.toSVG();
    let n = createFileName(this._config.PROJECT_NAME, "svg");
    downloadSVG(svg, n);
  }
