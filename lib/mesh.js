...
class MeshJS {
  constructor() {
    ...
    this._printConfigKey = "c";

    this._keyCommands = new Map([
      ["p", "Save as a PNG"],
      ["v", "Download video if enabled"],
      ["c", "Print config to console"],
      ["j", "Save config as JSON"],
      ["SPACE", "Toggle pause"],
      ["i", "Run init"]
    ]);
  ...
  init(config, initCallback, drawCallback, promise) {
    ...
    let f = function() {
      if (config.PRINT_CONFIG_ON_START) {
        this.printConfig();
      }
  ...
  setPaused(paused) {
    this._paused = paused;
    console.log(this._paused ? "meshjs: Paused" : "meshjs: Resumed");
  }
  ...
  onKeyUp(event) {
    ...
    if (key === this._downloadPngKey) {
      ...
      this.callInitOnKey();
    } else if (key === this._printConfigKey) {
      this.printConfig();
    }
  }
  ...
  callInitOnKey() {
    ...
    //todo: update this so you can call and render even when
  ...
  printConfig() {
    ...
    console.groupEnd();
    //console.dir("CONFIG: " + JSON.stringify(this._config, null, 2));
  }
