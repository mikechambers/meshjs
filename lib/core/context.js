...
export default class Context2D {
  constructor(
    ...
    this._captureSVG = captureSVG;

    //todo: if captureSVG is set to true, then we should override BATCH_DRAW_COMMANDS
    //and print a message to console
    ...
    if (!this.backgroundColor) {
      ...
      ctx.fillStyle = this._backgroundColor.toHex();
  ...
  clear() {
    ...
    if (this._backgroundColor === undefined) {
      ...
      if (this._captureSVG) {
        this.fillStyle = this._backgroundColor.toHex();
  ...
  _clearCommands() {
    if (this._captureSVG && this._commands.length > 0) {
      this._frames.push(this._commands);
    } else {
  ...
  /*** convienence drawing apis (mostly to enabled better svg export) *********/
  ...
  drawCircle(circle) {
    ...
    let doFill = circle.fillColor !== undefined;
    let doStroke = circle.lineWidth && circle.strokeColor !== undefined;
    if (doFill) {
      this.fillStyle =
        circle.fillColor === undefined ? undefined : circle.fillColor.toCSS();
    }
  ...
  drawRectangle(rectangle) {
    let c = this._pool.get();
    c.name = "drawRectangle";
    c.args = rectangle;

    let doFill = rectangle.fillColor !== undefined;
    let doStroke = rectangle.lineWidth && rectangle.strokeColor !== undefined;
    if (doFill) {
      this.fillStyle =
        rectangle.fillColor === undefined
          ? undefined
          : rectangle.fillColor.toCSS();
    }
