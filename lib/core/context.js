export default class Context2D {
  ...
  save(...args) {
    if (this._filterDrawCommands) {
      this._filterDrawCommands = false;
      console.log(
        "meshjs : context : Use of FILTER_DRAW_COMMANDS not currently supported " +
          "when saving transform state via context.save(). Disabling FILTER_DRAW_COMMANDS"
      );
    }
