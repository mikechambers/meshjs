...
export default class Branch {
  constructor(start, end) {
    ...
    this._maxRotationAngle = Math.PI / 6;
  }

  updateLeafColor(color) {
    this._leafColor = color;

    for (let c of this._children) {
      c.updateLeafColor(color);
    }
  }
  ...
  jitter() {
    this._end.add(Vector.createRandomVelocity());
  ...
  spawn() {
    ...
    let b1 = this.createChild(random(0, this._maxRotationAngle));
    let b2 = this.createChild(-random(0, this._maxRotationAngle));
